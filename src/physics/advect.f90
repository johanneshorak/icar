...
!
    subroutine advect3d(q,u,v,w,rho,dz,nx,nz,ny,options,bctop)
        ...
        real, dimension(1:nx,1:ny)      :: qinbc ! equal to qin if bc_top is not set or set to zero
        ...
        if (bctop /= 0) then
            ...
            elseif (bctop == 3) then
                ...
                where (w(:,nz,:) < 0) qinbc(:,:) = (w(:,nz-1,:)/w(:,nz,:)) * qin(:,nz,:)
            elseif (bctop == 4) then
                ! constant flux gradient
                ! four cases need to be distinguised based on w in the
                ! topmost three levels. the second derivative of the
                ! fluxes is calculated with the 2nd order backward
                ! difference scheme.
                qinbc(:,:) = (1/w(:,nz,:))*(2*qin(:,nz,:)*w(:,nz-1,:)-qin(:,nz-1,:)*w(:,nz-2,:)) ! case 1

                where ( (w(:,nz-1,:) < 0) .and. (w(:,nz-2,:) > 0) ) ! case 2
                    qinbc(:,:) = (1/w(:,nz,:))*(2*qin(:,nz,:)   * w(:,nz-1,:) - qin(:,nz-2,:)*w(:,nz-2,:))
                end where

                where ( (w(:,nz-1,:) > 0) .and. (w(:,nz-2,:) < 0) ) ! case 3
                    qinbc(:,:) = (1/w(:,nz,:))*(2*qin(:,nz-1,:) * w(:,nz-1,:) - qin(:,nz-1,:)*w(:,nz-2,:))
                end where

                where ( (w(:,nz-1,:) > 0) .and. (w(:,nz-2,:) > 0) )! case 4
                    qinbc(:,:) = (1/w(:,nz,:))*(2*qin(:,nz-1,:) * w(:,nz-1,:) - qin(:,nz-2,:)*w(:,nz-2,:))
                end where
        ...
        do i=2,ny-1
           ...
           if (options%advect_density) then
               ...
               q(2:nx-1,nz,i)     = q(2:nx-1,nz,i) - (qinbc(2:nx-1,i) * w(2:nx-1,nz,i) - f5(:,nz-1))      &
    ...
    subroutine advect_cu_winds(domain, options, dt)
        ...
        call advect3d(domain%u_cu, U_4cu_u,V_4cu_u,W_4cu_u, domain%rho, domain%dz_inter, nx+1,nz,ny, options, 0)
        call advect3d(domain%v_cu, U_4cu_v,V_4cu_v,W_4cu_v, domain%rho, domain%dz_inter, nx,nz,ny+1, options, 0)
    ...
    subroutine upwind(domain,options,dt)
        ...
        ! 20190402 jh ... added the boundary condition to be used at the model top to the function call
        call advect3d(domain%qv,          U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_qv)
        call advect3d(domain%cloud,       U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_sushyd)
        call advect3d(domain%qrain,       U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
        call advect3d(domain%qsnow,       U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
        call advect3d(domain%th,          U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_th)
        if (options%physics%microphysics == kMP_THOMPSON) then
            call advect3d(domain%ice,     U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_sushyd)
            call advect3d(domain%qgrau,   U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
            call advect3d(domain%nice,    U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_sushyd)
            call advect3d(domain%nrain,   U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
        elseif (options%physics%microphysics == kMP_MORRISON) then                
            call advect3d(domain%ice,     U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_sushyd)
            call advect3d(domain%qgrau,   U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
            call advect3d(domain%nice,    U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_sushyd)
            call advect3d(domain%nrain,   U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
            call advect3d(domain%nsnow,   U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
            call advect3d(domain%ngraupel,U_m,V_m,W_m, domain%rho, domain%dz_inter, nx,nz,ny, options,options%adv_options%bct_prechyd)
